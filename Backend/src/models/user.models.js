...
userschema.pre("save", async function (next) {
  //.pre is a middleware offered by mongoose which says " before saving the document run the function ", also we didn't used arrow function here because it does not contain the current context refernce or this cant be used there.
...
userschema.methods.isPasswordCorrect = async function (password) {
  //it adds a method to the userschema
...
userschema.methods.generateAccessToken = function () {
  return jwt.sign(
    //jwt.sign helps create a token which will in future be used to check whom to give data and whom to not(jiske paas token hoga usko data de denge)
    {
