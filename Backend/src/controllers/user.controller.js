...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Somthing went wrong will generation access and refresh tokens"
    );
  }
};
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const isalreadyuser = await User.findOne(
    //findone is used to fint the first document that matches the condition and used await beacuse database is in different continent
    {
  ...
  if (isalreadyuser)
    throw new ApiError(409, "User with same email id or username exists");
  ...
  const userCreated = await User.create({
    //this created an entry in mongodb
  ...
  //7
  const responseuser = await User.findById(userCreated._id).select(
    "-password -refreshToken"
  ); //disselected the fields which we didn't want to send back to the user as a response
  ...
  return res
    .status(201)
    .json(new ApiResponse(200, responseuser, "User registered successfully")); //res.status is for the HTTP status code and apiresponse if for the application
});
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  const options = {
    // by doing this the cookie we are about to sent in the next step will be secured: cannot be edited from the frontend, can only be edited from the server.
    httpOnly: true,
    secure: false,
    sameSite: "lax",
    path: "/",
...
const logoutUser = asyncHandler(async (req, res) => {
  await findByIdAndUpdate(
    //using this we can update something in database quickly and efficiently
    req.user._id,
    {
      $unset: {
        //removes values in database
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  ); //this helps to verify that the refreshtoken recived is created by the same jwt.sign(patternwaise).recived refresh token can be old or new here
  ...
  if (recivedRefreshToken !== user?.refreshToken) {
    //this helps here that the refresh token recived is new or not/ same as the one in DB.
...
const changeCurrentPsaaword = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(201, {}, "Password is updated"));
});
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(201, user, "Account details has been updated"));
});
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(201, user, "Cover Image is Updated");
});
...
const getUserProfileData = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    {
      $match: {
        //User me jake username.toLowerCase naam ke document ko username me daal dega.
        username: username.toLowerCase,
      },
    },
    {
      $lookup: {
        //if the _id id matched with the channel id the whole document from the subscriptions modle will be inserted int eh subscribers array
        from: "subscriptions",
        localField: "_id",
    ...
    {
      $project: {
        //this stage helps us select which feild to include and which to exlude
        fullName: 1,
        username: 1,
        avatar: 1,
        coverImage: 1,
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, channel[0], "Profile data fetched successfully")
    );
});
