...
export default function PlaylistPage() {
  ...
  const items = useMemo(
    () => videos.filter((v) => v.playlistId === playlistId || playlistId === "default").slice(0, 6),
    [videos, playlistId],
  )
    ...
    <main className="p-4 md:p-6 grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6">
      <Card className="h-fit sticky top-4 self-start transition-all">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="text-sm">A curated set of videos. ID: {playlistId}</div>
        </CardContent>
      </Card>
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {items.map((v) => (
            <div key={v.id} className="transition-transform hover:-translate-y-0.5">
              <VideoCard
                title={v.title}
